...
- (id) fl_searchModel:(Class)modelClass byColumName:(NSString *)columName ColumValue:(NSString *)columValue AutoCloseDB:(BOOL)autoCloseDB {
    if ([FLCURRENTDB open]) {
        if(![self isExitTable:modelClass autoCloseDB:NO]){
            if (autoCloseDB) {
                [FLCURRENTDB close];
        ...
        // 查询数据
        FMResultSet *rs = [FLCURRENTDB executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ = '%@';",modelClass,columName , columValue]];
        // 创建对象
        id object = nil;
        // 遍历结果集
        while ([rs next]) {
